...
public class ProveedoresJpaController implements Serializable {
   ...
   DefaultTableModel modelo2;
    ...
    public DefaultTableModel listarProveedores(){
        crearModeloItem();
        ProveedoresJpaController bienesB = new ProveedoresJpaController();
        try{
            Object O[]=null;
            List<Proveedores> e = bienesB.findProveedoresEntities();
            for (int i=0; i < e.size(); i++){
                modelo2.addRow(O);
                modelo2.setValueAt(e.get(i).getNit(), i, 0);
                modelo2.setValueAt(e.get(i).getNombre(), i, 1);
                modelo2.setValueAt(e.get(i).getTelefono(), i, 2);
            }
    }catch (Exception e) {
