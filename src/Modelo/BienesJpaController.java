...
public class BienesJpaController implements Serializable {
    ...
    public void crearModeloItem() {
        try {
            modelo2 = (new DefaultTableModel(
                    null, new String[]{
                        "cod_ITEM", "Nombre",
                        "Proveedor", "Valor Unitario"}) {
    ...
    public DefaultTableModel listarItems(){
        crearModeloItem();
        BienesJpaController bienesB = new BienesJpaController();
        try{
            Object O[]=null;
            List<Bienes> e = bienesB.findBienesEntities();
            for (int i=0; i < e.size(); i++){
                modelo2.addRow(O);
                modelo2.setValueAt(e.get(i).getCodItem(), i, 0);
                modelo2.setValueAt(e.get(i).getNombre(), i, 1);
                modelo2.setValueAt(e.get(i).getProveedor().getNit(), i, 2);
                modelo2.setValueAt(e.get(i).getValorUnitario(), i, 3);
            }
    }catch (Exception e) {
    ...
    public DefaultTableModel BuscarItems(int codigo){
        crearModeloItem();
        BienesJpaController bienesB = new BienesJpaController();
        try{
            Object O[]=null;
